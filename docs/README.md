...
The `INITB` signal is also routed to LED D10, in addition to being monitored by the controller. You can configure the FPGA to perform an automatic "continuous CRC" function which is useful for fault injection work. This flag can be monitored on the Python API.
